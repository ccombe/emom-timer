...
describe("Core Logic", () => {
  ...
  describe("Round Calculation", () => {
    ...
    it("identifies rounds correctly", () => {
      // Start (0s) -> Round 1
      expect(
        getCurrentRound({ elapsed: 0, intervalDuration: intervalSecs, totalDuration }),
      ).toEqual({ current: 1, total: 5 });

      // 59s -> Round 1
      expect(
        getCurrentRound({ elapsed: 59, intervalDuration: intervalSecs, totalDuration }),
      ).toEqual({ current: 1, total: 5 });

      // 60s -> Round 2 (Start of 2nd minute)
      expect(
        getCurrentRound({ elapsed: 60, intervalDuration: intervalSecs, totalDuration }),
      ).toEqual({ current: 2, total: 5 });

      // 299s -> Round 5
      expect(
        getCurrentRound({ elapsed: 299, intervalDuration: intervalSecs, totalDuration }),
      ).toEqual({ current: 5, total: 5 });
    });

    it("handles finished state", () => {
      expect(
        getCurrentRound({ elapsed: 300, intervalDuration: intervalSecs, totalDuration }),
      ).toEqual({ current: 5, total: 5 });
    });

    it("handles very large elapsed time gracefully", () => {
      expect(
        getCurrentRound({ elapsed: 1000, intervalDuration: intervalSecs, totalDuration }),
      ).toEqual({ current: 5, total: 5 });
    });
  });
